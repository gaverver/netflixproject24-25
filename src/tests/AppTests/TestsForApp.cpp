...
AppTester build() {
    ...
    DBFile dbfile("../data");
    IDataBase& data = dbfile;
    addCommand* add = new addCommand(data);
    recommendCommand* recommend = new recommendCommand(data, menu);
    helpCommand* help = new helpCommand(helpCommands, &menu);
...
std::string captureOutput(AppTester Runner, std::vector<std::string> commands) {
...
TEST(AppTests, HelpCommandTest) {
    ...
    std::string printedOutput = captureOutput(app, commands);
...
TEST(AppTests, NonVolatileDataTest) {
    ...
    std::vector<std::string> commands = {"add 9 100 103 105 107 112 113 115", "add 10 100 102 105 106 107 109 110 116", "recommend 1 104"};
    // capture the output from the cout
    std::string printedOutput = captureOutput(app, commands);
...
TEST(AppTests, DuplicateMoviesTest) {
    ...
    std::vector<std::string> expectedMovies2 = {"101", "102", "104", "105", "106"};
...
TEST(AppTests, InvalidInputTest1) {
    ...
    std::vector<std::string> expectedMovies1 = {"1"};
