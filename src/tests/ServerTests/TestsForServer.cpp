...
TEST(ServerTesting, MultipleClientExecutionTest) {
    ...
    std::thread thread1(clientFunction, {"POST 1 100\n", "PATCH 1 101\n"}, expectedOutput12);
    std::thread thread2(clientFunction, {"POST 2 100\n", "PATCH 2 101\n"}, expectedOutput12);
    std::thread thread3(clientFunction, {"help\n"}, expectedOutput3);
    std::thread thread4(clientFunction, {"DELETE 2 100 101 c g f\n", "DELETE 1 100 X 101\n"}, expectedOutput4);
...
TEST(ServerTesting, POSTPATCHGETTest) {
    ...
    std::thread thread1(clientFunction, {"POST 1 1\n"}, expectedOutput1);
    std::thread thread2(clientFunction, {"POST 2 1\n", "PATCH 2 3\n"}, expectedOutput2);
    std::thread thread3(clientFunction, {"POST 3 2\n"}, expectedOutput3);
    // waits for the clients to finish execution
    thread1.join();
    thread2.join();
    thread3.join();
    ...
    std::thread GetThread1(clientFunction, {"GET 1 1\n"}, expectedOutput1);
    std::thread GetThread2(clientFunction, {"GET 1 2\n"}, expectedOutput2);
    std::thread GetThread3(clientFunction, {"GET 1 3\n"}, expectedOutput3);
    // waits for the clients to finish execution
    GetThread1.join();
    GetThread2.join();
    GetThread3.join();
...
TEST(ServerTesting, POSTPATCHDELETETest) {
    ...
    std::string expectedOutput1 = "201 Created\n204 No Content\n204 No Content\n";
    std::thread thread1(clientFunction, {"POST 1 1\n", "PATCH 1 5 6 8 8 9\n", "PATCH 1 70 70 80 90\n"}, expectedOutput1);
    std::thread thread2(clientFunction, {"POST 2 1 1\n", "PATCH 2 3\n"}, expectedOutput2);
    ...
    EXPECT_EQ(movies1, {1, 5, 6, 8, 9, 70, 80, 90});
    expectedDeleteOutput = "204 No Content\n204 No Content\n";
    ...
    std::thread DeleteThread1(clientFunction, {"DELETE 1 1 5 8\n", "DELETE 2 1 3\n"}, e);
    std::thread DeleteThread2(clientFunction, {"DELETE 1 70 80\n", "DELETE 3 10\n"}, e);
    std::thread DeleteThread3(clientFunction, {"DELETE 3 11\n", "DELETE 3 12\n"}, e);
    // waits for the clients to finish execution
    DeleteThread1.join();
    DeleteThread2.join();
    DeleteThread3.join();
...
TEST(ServerTesting, InvalidInputTest) {
    ...
    std::thread thread1(clientFunction, {"foo\n", "help x\n", "PATCH 10\n", "post 20\n", "DELETE 4 C\n"}, expectedOutput1);
    std::thread thread2(clientFunction, {"PATCH 1 2\n", "POST 1 2\n", "POST 1 3\n"}, expectedOutput2);
    std::thread thread3(clientFunction, {"DELETE 1 2 3\n", "GET 2 1\n", "GET 1\n", "help\n"}, expectedOutput3);
    // waits for the clients to finish execution
    thread1.join();
    thread2.join();
    thread3.join();
